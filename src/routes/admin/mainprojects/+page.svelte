		...
		<div class="mx-auto max-w-4xl p-6 md:p-12">
			<div class="grid gap-6">
				{#each projects as _, i}
					<button
						type="button"
						class="group relative w-full cursor-pointer text-left"
						onclick={() => (editingIndex = i)}
					>
						<ProjectCard bind:project={projects[i]} index={i} total={projects.length} />
						<div
							class="pointer-events-none absolute inset-0 rounded-2xl bg-emerald-500/0 transition-colors group-hover:bg-emerald-500/[0.02]"
						></div>
					</button>
				{/each}
			</div>
			...
			<div class="custom-scrollbar overflow-y-auto p-10">
				...
				<div class="space-y-8">
					<div class="space-y-2">
						...
						<input
							id="project-title"
							bind:value={projects[editingIndex].title}
					...
					<div class="space-y-2">
						...
						<textarea
							...
							bind:value={projects[editingIndex].description}
							rows="5"
					...
					<div class="grid grid-cols-2 gap-6">
						<div class="space-y-2">
							...
							<input
								id="project-image"
								bind:value={projects[editingIndex].image}
								class="w-full rounded-2xl border border-white/5 bg-white/5 p-4 text-white outline-none focus:ring-1 focus:ring-emerald-500"
							/>
						</div>
						<div class="space-y-2">
							...
							<input
								id="project-link"
								bind:value={projects[editingIndex].link}
								class="w-full rounded-2xl border border-white/5 bg-white/5 p-4 text-white outline-none focus:ring-1 focus:ring-emerald-500"
							/>
						</div>
					</div>
					...
					<div class="space-y-2">
						...
						<input
							id="project-tags"
							value={projects[editingIndex].tags?.join(', ')}
							oninput={(e) =>
								(projects[editingIndex!].tags = (e.target as HTMLInputElement).value
									.split(',')
									.map((s) => s.trim())
									.filter(Boolean))}
							class="w-full rounded-2xl border border-white/5 bg-white/5 p-4 text-white outline-none focus:ring-1 focus:ring-emerald-500"
